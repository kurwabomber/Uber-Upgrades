		...
		if(IsValidWeapon(CWeapon)){
			if(immolationActive[client]){
				...
				if(immolationRatio > 0.0){
					SDKHooks_TakeDamage(client, client, client, TF2Util_GetEntityMaxHealth(client)*immolationRatio*0.1, DMG_PREVENT_PHYSICS_FORCE|DMG_IGNOREHOOK|DMG_PIERCING);
				}
			}
			if(sunstarDuration[client] >= GetGameTime()){
				...
				if(sunstarActive == 15.0){
					...
					while ((i = FindEntityByClassname(i, "*")) != -1)
					{
						...
						SDKHooks_TakeDamage(i,client,client,LightningDamage,DMG_SHOCK|DMG_IGNOREHOOK,CWeapon,_,_,false);
					}
				}
			}
		}
		...
		if (IsValidClient3(client) && IsPlayerAlive(client))
		{
			...
			if(TF2Attrib_HookValueFloat(0.0, "revenge_powerup", client) == 1)
			{
				if(RageBuildup[client] < 1.0)
					Format(StatusEffectText, sizeof(StatusEffectText),"%sRevenge: %.0f%%%%\n", StatusEffectText, RageBuildup[client]*100.0);
				else
					Format(StatusEffectText, sizeof(StatusEffectText),"%sRevenge: READY (Crouch + Mouse3)\n", StatusEffectText, RageBuildup[client]*100.0);
			...
			{
				Format(StatusEffectText, sizeof(StatusEffectText),"%sBerserk: %.0f%%%%\n", StatusEffectText, RageBuildup[client]*100.0);
			...
			if(TF2Attrib_HookValueFloat(0.0, "supernova_powerup", client) == 1)
			{
				if(SupernovaBuildup[client] < 1.0)
					Format(StatusEffectText, sizeof(StatusEffectText),"%sSupernova: %.0f%%%%\n", StatusEffectText, SupernovaBuildup[client]*100.0);
				else
					Format(StatusEffectText, sizeof(StatusEffectText),"%sSupernova: READY (Crouch + Mouse3)\n", StatusEffectText);
			}
			else if(TF2Attrib_HookValueFloat(0.0, "supernova_powerup", client) == 2)
			{
				if(infernalDetonationCooldown[client]-GetGameTime() > 0)
					Format(StatusEffectText, sizeof(StatusEffectText),"%sDetonate: %.2fs\n", StatusEffectText, infernalDetonationCooldown[client]-GetGameTime());
				else
					Format(StatusEffectText, sizeof(StatusEffectText),"%sDetonate: READY (Crouch + Mouse3)\n", StatusEffectText);
			}
			if(TF2Attrib_HookValueFloat(0.0, "regeneration_powerup", client) == 2.0)
			{
				if(duplicationCooldown[client] > GetGameTime())
					Format(StatusEffectText, sizeof(StatusEffectText),"%sDuplication: %.2fs\n", StatusEffectText, duplicationCooldown[client] - GetGameTime());
				else
					Format(StatusEffectText, sizeof(StatusEffectText),"%sDuplication: READY (Crouch + Mouse3)\n", StatusEffectText);
			}
			if(TF2Attrib_HookValueFloat(0.0, "agility_powerup", client) == 3.0)
			{
				if(warpCooldown[client] > GetGameTime())
					Format(StatusEffectText, sizeof(StatusEffectText),"%sWarp: %.2fs\n", StatusEffectText, warpCooldown[client] - GetGameTime());
				else
					Format(StatusEffectText, sizeof(StatusEffectText),"%sWarp: READY (Crouch + Mouse3)\n", StatusEffectText);
			}
			if(TF2Attrib_HookValueFloat(0.0, "resistance_powerup", client) == 2.0)
			{
				if(frayNextTime[client] > GetGameTime())
					Format(StatusEffectText, sizeof(StatusEffectText),"%sFray: %.2fs\n", StatusEffectText, frayNextTime[client] - GetGameTime());
				else
					Format(StatusEffectText, sizeof(StatusEffectText),"%sFray: READY\n", StatusEffectText);
			}
			else if(TF2Attrib_HookValueFloat(0.0, "resistance_powerup", client) == 3.0)
			{
				if(!strongholdEnabled[client])
					Format(StatusEffectText, sizeof(StatusEffectText),"%sStronghold: INACTIVE (Crouch + Mouse3)\n", StatusEffectText);
				else
					Format(StatusEffectText, sizeof(StatusEffectText),"%sStronghold: ACTIVE (Crouch + Mouse3)\n", StatusEffectText);
			}
			if(TF2Attrib_HookValueFloat(0.0, "king_powerup", client) == 2.0)
			{
				if(!IsValidClient3(tagTeamTarget[client]))
					Format(StatusEffectText, sizeof(StatusEffectText),"%sTag-Team: INACTIVE (Crouch + Mouse3)\n", StatusEffectText);
				else
					Format(StatusEffectText, sizeof(StatusEffectText),"%sTag-Team: %N\n", StatusEffectText, tagTeamTarget[client]);
			}
			...
			if(hasBuffIndex(client, Buff_Decay)){
				...
				if(client != decay.inflictor && IsValidClient3(decay.inflictor) && IsOnDifferentTeams(client,decay.inflictor)){
					SDKHooks_TakeDamage(client, decay.inflictor, decay.inflictor, 10.0 + GetClientHealth(client)*0.008 * decay.severity, DMG_PREVENT_PHYSICS_FORCE|DMG_IGNOREHOOK|DMG_PIERCING);
					SDKHooks_TakeDamage(client, decay.inflictor, decay.inflictor, 9.0,DMG_RADIATION|DMG_DISSOLVE|DMG_IGNOREHOOK,_,_,_,false);
				}
			}
			if(hasBuffIndex(client, Buff_InfernalDOT)){
				...
				if(client != infernalDOT.inflictor && IsValidClient3(infernalDOT.inflictor)){
					SDKHooks_TakeDamage(client, infernalDOT.inflictor, infernalDOT.inflictor, InfernalEnchantment[infernalDOT.inflictor]*0.1 * infernalDOT.severity,DMG_IGNOREHOOK,_,_,_,false);
			...
			if(hasBuffIndex(client, Buff_LifeLink)){
				...
				if(IsValidClient3(lifelink.inflictor)){
					SDKHooks_TakeDamage(client, lifelink.inflictor, lifelink.inflictor, GetClientHealth(client)*0.0035 * lifelink.severity, DMG_PREVENT_PHYSICS_FORCE|DMG_IGNOREHOOK|DMG_PIERCING);
				}
			}
			if(hasBuffIndex(client, Buff_PowerupBurning)){
				...
				if(client != infernalDOT.inflictor && IsValidClient3(infernalDOT.inflictor)){
					SDKHooks_TakeDamage(client, infernalDOT.inflictor, infernalDOT.inflictor, 10.0 * infernalDOT.severity, DMG_PREVENT_PHYSICS_FORCE|DMG_IGNOREHOOK|DMG_PIERCING);
					if(hitParticle[client]+0.4 <= GetGameTime()){
						CreateParticleEx(client, "halloween_burningplayer_flyingbits", 1);
						hitParticle[client] = GetGameTime()
					}
			...
			if(hasBuffIndex(client, Buff_Plagued))
			{
				...
				if(IsValidClient3(plagueCond.inflictor)){
					SDKHooks_TakeDamage(client, plagueCond.inflictor, plagueCond.inflictor, plagueCond.severity*3, DMG_PREVENT_PHYSICS_FORCE|DMG_IGNOREHOOK|DMG_PIERCING);
				}
			}
			if(IsValidWeapon(CWeapon))
			{
								...
								while ((i = FindEntityByClassname(i, "*")) != -1)
								{
									if(IsValidForDamage(i) && IsOnDifferentTeams(client,i))
									{
										...
										if(GetVectorDistance(clientpos,VictimPos,true) <= range*range*0.3)
											if(IsPointVisible(clientpos,VictimPos)){
												SDKHooks_TakeDamage(i,client,client, LightningDamage, DMG_IGNOREHOOK, secondary, _,_,false);
											}
									}
								}
							}
			...
			if(IsValidForDamage(i) && IsOnDifferentTeams(client,i))
			{
				...
				if(GetVectorDistance(fOrigin,VictimPos,true) <= 250000)
				{
					if(IsPointVisible(PlayerOrigin,VictimPos))
					{
						...
						SDKHooks_TakeDamage(i,client,client, mult, DMG_SLASH|DMG_IGNOREHOOK, weapon, _,_,false);
					}
				}
			}
		}
...
public Action RecursiveExplosions(Handle timer, DataPack ref) 
{
	...
	int entity = ref.ReadCell();
	float position[3];
	position[0] = ref.ReadFloat();
	position[1] = ref.ReadFloat();
	position[2] = ref.ReadFloat();

	if(IsValidClient3(owner) && IsValidWeapon(weapon) && entity > 0 && entity < MAXENTITIES && recursiveExplosionCount[entity] < 10){
		...
		if(chance >= GetRandomFloat(0.0,1.0)){
			recursiveExplosionCount[entity]++;
			return Plugin_Continue;
		}
	}
