...
public void OnPluginStart() {
	...
	g_hCvarPlayerControlEnabled = CreateConVar("tf2items_manager_playercontrol", "0", "Enables/disables the player's ability to control the manager (0 - Disabled / 1 - Enabled");
...
public Action TF2Items_OnGiveNamedItem(int client, char[] classname, int itemDefIndex, Handle &override) {
	...
	if (item != null) {
		if(TF2Items_GetFlags(item) & OVERRIDE_ITEM_DEF){
			DataPack pack = new DataPack();
			pack.WriteCell(client);
			pack.WriteCell(view_as<int>(item));
			RequestFrame(CheckEquip, pack);
			return Plugin_Stop;
		}
		override = item;
...
public void CheckEquip(DataPack pack)
{
	pack.Reset();

	int client = pack.ReadCell();
	Handle item = view_as<Handle>(pack.ReadCell());

	if(IsValidClient(client)){
		TF2_RemoveWeaponSlot(client, TF2Econ_GetItemLoadoutSlot(TF2Items_GetItemIndex(item), TF2_GetPlayerClass(client)));
		int ent = TF2Items_GiveNamedItem(client, item);
...
void ParseItemsEntry(KeyValues kv, Handle entry) {
	char buffer[64], buffer2[64], explBuffer[16][64], split[2][64];
		...
		do {			
			...
			for(int i = 0;i < amountItems; ++i)
			{
				...
				if (buffer[0] == '*')
					...
					TF2Items_SetItemIndex(item, StringToInt(explBuffer[i]));
